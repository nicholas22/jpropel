...
public final class ConfigurableParameters
{
   // file of similar functionality to app.config in .NET
	 ...
	 * If no arguments are passed, it uses the default constructor, otherwise an overloaded version.
	 *
	 ...
	 * If an error occurs, this method will log the problem and return an empty collection.
	 *
	 * Examples:
	 * <ul>
	 * <li>(sep= )[1 2 3] </li>
  ...
  public static <T> Iterable<T> getIterable(KeyValuePair<String, ReifiedIterable<T>> kvp)
			throws ConfigurationErrorsException
	{
		if(StringUtils.contains(lookup.getKeys(), kvp.getKey(), StringComparison.Ordinal))
		{
			...
			if(value != null && value.length() > SEP_PREFIX.length() + 2 && StringUtils.startsWith(value, SEP_PREFIX, StringComparison.OrdinalIgnoreCase)) // e.g. (sep=,)
			{
				...
				if(StringUtils.startsWith(value, CAST_PREFIX) && value.contains(CONSTANT.COMMA) && StringUtils.substring(value, value.indexOf(CONSTANT.COMMA)).contains(CONSTANT.CLOSE_PARENTHESIS))
				{
				...
				if(fromTypeName == null || toTypeName == null)
				{
					...
					{
						throw new ConfigurationErrorsException("The '" + kvp.getKey() + "' configurable enumerable values contained an invalid element: " + val, e);
					}
				}
				else
				{
					...
					//if(toType != typeof(T))
					//	throw new ConfigurationErrorsException("The '" + kvp.Key + "' configured enumerable cast (" + toType.Name + ") must match T (" + typeof(T).Name + ") for casting to take place.");
					...
					for(String val : values)
					{
						...
						{
							throw new ConfigurationErrorsException("The '" + kvp.getKey() + "' configurable enumerable values contained an element that could not be re-cast: " + val, e);
						}
					}
				}
				...
				throw new ConfigurationErrorsException("The format of '" + kvp.getKey() + "' configurable enumerable value should look like this: (sep= )(cast=System.Int32,System.Char)[1 2 3 4]");
		}
