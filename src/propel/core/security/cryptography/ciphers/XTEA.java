/*
 * Copyright 2004-2011 H2 Group. Multiple-Licensed under the H2 License, Version 1.0, and under the Eclipse Public License, Version 1.0
...
public class XTEA
{
  ...
  public void encrypt(byte[] bytesIn, byte[] bytesOut, int off, int len)
  {
    for (int i = off; i < off + len; i += 8)
      encryptBlock(bytesIn, bytesOut, i);
  }

  public void decrypt(byte[] bytesIn, byte[] bytesOut, int off, int len)
  {
    for (int i = off; i < off + len; i += 8)
      decryptBlock(bytesIn, bytesOut, i);
  }
