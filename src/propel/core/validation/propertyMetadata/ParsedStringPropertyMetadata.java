    ...
    extends StringPropertyMetadata
{
   ...
   * Constructor
   */
  public ParsedStringPropertyMetadata(String name, int minLength, int maxLength, boolean notNull, boolean notEmpty, boolean noNullChars, boolean onlyAsciiPrintableCharsAllowed)
  {
   ...
   */
  protected String trimWhitespaceChars(char[] value)
  {
    if (value.length == 0)
    ...
    while (isTrimmable(value[start]) && start < value.length - 1)
      start++;
    while (isTrimmable(value[end]) && end > start)
      end--;

    int length = end + 1 - start;
    String result = new String(value, start, length);
   ...
   */
  protected void checkOnlyAsciiPrintableChars(char[] value)
      throws ValidationException 
  {
    for(char ch : value) 
      if(ch < 32 || ch > 126)
