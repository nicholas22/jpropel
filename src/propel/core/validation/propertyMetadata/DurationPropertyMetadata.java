		...
		extends NullablePropertyMetadata<Duration>
{
			...
			throws ValidationException
	{
		...
		if(value.compareTo(getMaxValue()) > 0)
			throw new ValidationException(String.format(BoundedValueTypePropertyMetadata.SHOULD_NOT_BE_GREATER_THAN, getName()) + getMaxValue().getStandardSeconds()+" secs.");

		if(value.compareTo(getMinValue()) < 0)
			throw new ValidationException(String.format(BoundedValueTypePropertyMetadata.SHOULD_NOT_BE_LESS_THAN, getName()) + getMinValue().getStandardSeconds()+" secs.");
	}
}
