		...
		extends EncodedStringPropertyMetadata
{
	...
	{
		this(name, ConfigurableParameters.tryGetIterable(ConfigurableConsts.VALIDATION_ALLOWED_IPV6_CHARS), DEFAULT_MIN_IPV6_LENGTH, DEFAULT_MAX_IPV6_LENGTH, notNull, notEmpty);
	}
	 ...
	 * @throws IllegalArgumentException An argument is invalid
	 */
	public IpV6AddressPropertyMetadata(String name, Iterable<Character> allowedChars, int minLength, int maxLength, boolean notNull, boolean notEmpty)
	{
			...
			throws ValidationException
	{
		...
		if((numOfColons >= 2) && (numOfColons <= MAX_NUM_HEX_GROUPS - 1))
		{
			...
			if(numOfDoubleColons == 1)
			{
				...
				{
					String[] firstGroups = StringUtils.split(value.substring(0, value.length() - CONSTANT.DOUBLE_COLON.length()), CONSTANT.COLON_CHAR);
